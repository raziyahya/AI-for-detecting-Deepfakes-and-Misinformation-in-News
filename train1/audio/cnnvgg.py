...
# Step 7: Define the VGG-like CNN model
...
model = create_vgg_like_model(input_shape)

# Step 9: Train the model
history = model.fit(X_train, y_train_cat, epochs=17, batch_size=32, validation_data=(X_test, y_test_cat))
model.save('vgg_audio_model')  # Saves the model in HDF5 format
# Step 10: Evaluate the model
test_loss, test_accuracy = model.evaluate(X_test, y_test_cat)
print(f'Test Accuracy: {test_accuracy * 100:.2f}%')
print(history.history)
# Optionally, you can plot the training history to visualize learning
import matplotlib.pyplot as plt
